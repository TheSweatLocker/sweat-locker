...
{gamesSport==='MLB'&&(()=>{
  ...
  const nColor = nScore >= 88 ? '#00e5a0' : nScore >= 70 ? '#00e5a0' : nScore <= 40 ? '#ff4d6d' : '#7a92a8';
  const nLabel = nScore >= 88 ? 'NRFI 🔒' : nScore >= 70 ? 'NRFI' : nScore <= 40 ? 'YRFI' : 'Neutral';
  return(
...
const nrfiLean  = nrfiScore >= 88 ? 'NRFI 🔒' : nrfiScore >= 70 ? 'NRFI' : nrfiScore <= 40 ? 'YRFI' : 'NEUTRAL';
