    ...
    for sport, tbl in PROP_TABLE_BY_SPORT.items():
        ...
        for row in rows:
            ...
            if _pt in ('hits_over', 'hits_under'):
                continue
            # 2026-09-17 SHARED POLICY via prop_ban_policy.py — single
            # source of truth for batter-family bans across every
            # composer. Same import used by prop_jerry_synth, sweat_card,
            # sharp_card, daily_degen.
            from prop_ban_policy import is_banned_mlb_prop
            if sport == 'MLB' and is_banned_mlb_prop(_pt, row.get('tier')):
                continue
...
def run(game_date: str | None = None, threshold: int = 70,
    ...
    # 2026-09-19 Andy: morning output stays. 15 → 12, in step with the
    # Sweat Card, Sharp Card and MLB_PICK_LOCK_ET_HOUR. POTD rides on the
    # Sweat Card, so a later lock here would let the headline play move
    # after the card holding it had already frozen.
    _HARD_LOCK_HOUR = int(os.environ.get('POTD_HARD_LOCK_ET_HOUR', '12'))
