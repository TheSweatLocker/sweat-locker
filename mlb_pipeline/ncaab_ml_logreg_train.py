"""NCAAB ML logistic regression trainer. Rich feature set: KenPom-style
adjusted efficiency + market lines. 5911 resolved games available."""
...
FEATURES = ['close_spread', 'open_spread', 'close_total', 'open_total',
            'home_ml_close', 'away_ml_close', 'home_ml_open', 'away_ml_open',
            'projected_spread', 'projected_total', 'pace_avg',
            'home_adj_em', 'away_adj_em', 'adj_em_gap',
...
def train(dry_run=False):
    ...
    print(f'  {len(rows)} resolved NCAAB games')
    if len(rows) < 300:
    ...
    X = X[:, valid_col_mask]
    if X.shape[1] == 0:
        print('  ⚠ BLOCKED: no populated features. NCAAB results table lacks '
              'market lines (close_spread/close_total/*_ml_*) and KenPom '
              'efficiency (adj_em/adj_oe/adj_de). Backfill before training.')
        return
    ...
    print(f'\n  Top coefs:')
    for f, c in sorted(zip(features, lr.coef_[0]), key=lambda x: -abs(x[1]))[:8]:
        print(f'    {f:22s} {c:>+7.3f}')
    ...
    out_path = MODELS_DIR / 'ncaab_ml_logreg.json'
