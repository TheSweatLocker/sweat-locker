"""Backfill NFL team ATS/O/U/ML tendencies (2026-08-17).

Sport-specific port for NFL:
  * Window is L4 (17-game season, ~24% window)
  * Team names in NFL are 3-letter abbreviations (SEA, SF, KC)
  * Uses nfl_game_results.spread_result / total_result directly
  * 400-day lookback covers current + prior 2 seasons for season-splits

CLI:
  python backfill_nfl_team_tendencies.py                # today
  python backfill_nfl_team_tendencies.py --date 2026-09-07
  python backfill_nfl_team_tendencies.py --dry-run
"""
...
def compute_team_tendencies(games: list[dict]) -> dict:
    ...
    for team, entries in per_team.items():
        ...
        tendencies[team] = {
            'ats_last4': ats_covers, 'ats_last4_losses': ats_losses,
            'ou_last4_overs': overs, 'ou_last4_unders': unders,
            'ml_last4': ml_wins, 'ml_last4_losses': ml_losses,
...
def write_to_today(tendencies: dict, game_date: str, dry_run: bool = False) -> int:
    ...
    for g in games:
        ...
        patch = {
            'home_ats_last4':           home_tend.get('ats_last4'),
            'home_ats_last4_losses':    home_tend.get('ats_last4_losses'),
            'away_ats_last4':           away_tend.get('ats_last4'),
            'away_ats_last4_losses':    away_tend.get('ats_last4_losses'),
            'home_ou_last4_overs':      home_tend.get('ou_last4_overs'),
            'home_ou_last4_unders':     home_tend.get('ou_last4_unders'),
            'away_ou_last4_overs':      away_tend.get('ou_last4_overs'),
            'away_ou_last4_unders':     away_tend.get('ou_last4_unders'),
            ...
            'home_ml_last4':            home_tend.get('ml_last4'),
            'home_ml_last4_losses':     home_tend.get('ml_last4_losses'),
            'away_ml_last4':            away_tend.get('ml_last4'),
            'away_ml_last4_losses':     away_tend.get('ml_last4_losses'),
            'team_tendencies_updated_at': now_iso,
        }
        print(f'  {away_t:<5} @ {home_t:<5}  '
              f'ats={patch["away_ats_last4"]}-{patch["away_ats_last4_losses"]}/'
              f'{patch["home_ats_last4"]}-{patch["home_ats_last4_losses"]} '
              f'ou={patch["away_ou_last4_overs"]}o-{patch["away_ou_last4_unders"]}u/'
              f'{patch["home_ou_last4_overs"]}o-{patch["home_ou_last4_unders"]}u')
...
def run(game_date: str | None = None, dry_run: bool = False):
    ...
    print(f'=== backfill NFL team tendencies · {gd} ===')
    history = fetch_history(days_lookback=400)
    print(f'  fetched {len(history)} resolved NFL games (last 400d)')
