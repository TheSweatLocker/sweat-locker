"""NFL TOTAL logistic regression trainer (2026-09-08).

Trained on 6 seasons of nflverse-backfilled results (2020-2025 = 1,693
games). Fills the gap flagged in project_lr_totals_investigation_908:
NFL total LR was 0% coverage because no model file existed. This
creates one — same feature set as NCAAF/MLB total trainers.

USAGE:
    python nfl_total_logreg_train.py                # train + save model
    python nfl_total_logreg_train.py --dry-run      # print metrics, skip save
"""
...
def train(dry_run: bool = False):
    ...
    print(f'  pulled {len(rows)} resolved NFL games w/ scores + line')
    if len(rows) < 200:
    ...
    out_path = MODELS_DIR / 'nfl_total_logreg.json'
