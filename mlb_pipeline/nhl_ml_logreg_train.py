"""NHL ML logistic regression trainer. Thin market-only features but
1335 games available. NHL uses puckline (=spread) and totals."""
...
FEATURES = ['close_puckline', 'close_total', 'close_home_ml', 'close_away_ml']
...
def train(dry_run=False):
    ...
    print(f'  {len(rows)} resolved NHL games')
    ...
    X = X[:, valid_col_mask]
    if X.shape[1] == 0:
        print('  ⚠ BLOCKED: no populated features. NHL results table lacks '
              'close_puckline/close_total/close_home_ml/close_away_ml. '
    ...
    out_path = MODELS_DIR / 'nhl_ml_logreg.json'
