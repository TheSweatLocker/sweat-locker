"""Backfill NHL team ATS/O/U/ML tendencies (2026-08-17).

Sport-specific port for NHL:
  * Window is L5 (82-game season, ~2 weeks of form)
  * Team names in nhl_game_context are CITY only ("Boston", "Seattle")
    per NHL API convention — matches identifier throughout
  * Reads nhl_game_results.spread_result / total_result / home_win
  * 400-day lookback covers current + prior season

CLI:
  python backfill_nhl_team_tendencies.py                # today
  python backfill_nhl_team_tendencies.py --date 2026-10-08
  python backfill_nhl_team_tendencies.py --dry-run
"""
...
def fetch_history(days_lookback: int = 730) -> list[dict]:
    """Default 730d covers current + prior NHL season (Oct-Apr calendar)."""
    cutoff = (date.today() - timedelta(days=days_lookback)).isoformat()
    yesterday = (date.today() - timedelta(days=1)).isoformat()
    results = []
    for off in range(0, 20000, 1000):
        r = requests.get(
            f'{SB}/rest/v1/nhl_game_results'
            f'?game_date=gte.{cutoff}&game_date=lte.{yesterday}'
            '&select=game_id,game_date,home_team,away_team,home_score,away_score,'
            'close_puckline,close_total,spread_result,total_result,home_win'
...
def compute_team_tendencies(games: list[dict]) -> dict:
    ...
    for g in games:
        ...
        entry = {
            ...
            'close_puckline': g.get('close_puckline'),
    ...
    for team, entries in per_team.items():
        ...
        for e in entries:
            pl = e.get('close_puckline')
            if pl is None: continue
            try: pl = float(pl)
            except (TypeError, ValueError): continue
            team_pl = pl if e['as'] == 'HOME' else -pl
            sr = e.get('spread_result')
            team_covered = ((sr == 'home_covered' and e['as'] == 'HOME') or
                            (sr == 'away_covered' and e['as'] == 'AWAY'))
            if team_pl < 0:  # favorite (puckline negative)
                fav_games += 1
                if team_covered: fav_covers += 1
            elif team_pl > 0:  # underdog
                dog_games += 1
...
def write_to_today(tendencies: dict, game_date: str, dry_run: bool = False) -> int:
    ...
    for g in games:
        ...
        pr = requests.patch(
            f'{SB}/rest/v1/nhl_game_context?game_id=eq.{g["game_id"]}',
...
def run(game_date: str | None = None, dry_run: bool = False):
    ...
    print(f'=== backfill NHL team tendencies · {gd} ===')
    history = fetch_history(days_lookback=400)
    print(f'  fetched {len(history)} resolved NHL games (last 400d)')
