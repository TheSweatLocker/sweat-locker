...
def fetch_pipeline_props():
    ...
    try:
        ...
        pass
    # 2026-09-16 BAN POLICY (Andy TestFlight audit: "Daily Degen has Runs
    # over" which is on the permanent OVER ban list). Daily Degen reads
    # mlb_pipeline_props directly and bypasses the
    # generate_prop_jerry_synthesis _MLB_BANNED_PROP_TYPES filter — same
    # class of bug as Sweat Card. Mirror the policy exactly:
    ...
    def _keep(p: dict) -> bool:
        ...
        return True
    before = len(props)
    props = [p for p in props if _keep(p)]
    if before != len(props):
        print(f'  [daily_degen] ban filter dropped {before - len(props)} banned/sub-STRONG rows')
    return props
