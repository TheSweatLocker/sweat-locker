"""External picks pull — NCAAB (evening pre-slate + 30-min pre-tip refresh).

2026-07-23: Ports the NCAAF template (pull_externals_ncaaf.py) with
NCAAB sport-slug URLs. All shared scrapers (Playwright helper, source
registry shape, pull_log flow, ExternalPick dataclass) reused from the
same code paths — differences are:
  1. sport='NCAAB' throughout
  2. Sport slugs per site — CBB varies: /cbb/ (Dimers, BettingPros),
     /ncaab/ (Covers, Action), /college-basketball/ (VSiN, Pickswise),
     /college-basketball-picks (PickDawgz)
  3. Team-name aliases from ncaab_team_aliases (365 D1 basketball teams)
  4. Cadence: 5pm ET weekday primary + 60min pre-tip refresh (per project
     memory project_external_aggregation_launch — Nov-Mar season window)
  5. Non-D1 opponents may skip via alias miss — acceptable for v1

Source lineup (start subset — expand as we validate structure per site):
  BOOST:   Dimers CBB wp, Action Network sharp $
  TRUST:   Covers CBB consensus, BettingPros expert picks, PickDawgz free
  NEUTRAL: Pickswise 3-star, VSiN CBB Peterson
...
USAGE:
  python pull_externals_ncaab.py                        # weekly cron pull
  python pull_externals_ncaab.py --refresh              # Sat 10am refresh
  python pull_externals_ncaab.py --date 2026-09-06      # specific week
  python pull_externals_ncaab.py --dry-run              # print, no write
  python pull_externals_ncaab.py --source pickswise     # single source test
"""
import argparse
import os
import re
...
# ─────────────────────────────────────────────────────────────
# Source registry — NCAAB sport-slug per URL (varies per site)
# ─────────────────────────────────────────────────────────────
SOURCE_REGISTRY = {
    'dimers': {
        'fade_flag': 'boost', 'ttl_hours': 48,
        'base_url': 'https://www.dimers.com/bet-hub/cbb/schedule',
        'label': 'Dimers CBB',
    },
    'covers': {
        'fade_flag': 'trust', 'ttl_hours': 48,
        'base_url': 'https://contests.covers.com/consensus/topconsensus/ncaab/overall',
        'label': 'Covers CBB',
    },
    'action': {
        'fade_flag': 'trust', 'ttl_hours': 24,
        'base_url': 'https://www.actionnetwork.com/ncaab/public-betting',
        'label': 'Action Network CBB',
    },
    'vsin': {
        'fade_flag': 'trust', 'ttl_hours': 48,
        'base_url': 'https://vsin.com/college-basketball/',
        'label': 'VSiN CBB',
    },
    'bettingpros': {
        'fade_flag': 'trust', 'ttl_hours': 48,
        'base_url': 'https://www.bettingpros.com/cbb/picks/',
        'label': 'BettingPros CBB',
    },
    'pickswise': {
        'fade_flag': 'neutral', 'ttl_hours': 48,
        'base_url': 'https://www.pickswise.com/college-basketball/picks/',
        'label': 'Pickswise CBB',
    },
    'pickdawgz': {
        'fade_flag': 'trust', 'ttl_hours': 48,
        'base_url': 'https://www.pickdawgz.com/college-basketball-picks',
        'label': 'PickDawgz CBB',
    },
    'oddscrowd': {
        'fade_flag': 'trust', 'ttl_hours': 12,
        'base_url': 'https://oddscrowd.com/games/upcoming/basketball',
        'label': 'OddsCrowd CBB',
    },
    'scoresandodds': {
        'fade_flag': 'trust', 'ttl_hours': 12,   # 2026-08-25 SO scraper
        'base_url': 'https://www.scoresandodds.com/ncaab',
        'label': 'ScoresAndOdds CBB',
    },
}
...
# ─────────────────────────────────────────────────────────────
# NCAAB slate lookup — ncaab_game_context or ncaab_game_results fallback
# ─────────────────────────────────────────────────────────────
def load_slate(week_start: str) -> list:
    """Return list of upcoming NCAAB games in a ±2 day window from date.

    Reads ncaab_game_context first (in-season), falls back to ncaab_game_results.
    CBB cadence is daily during season — narrower window than NFL/NCAAF to
    match the same-day/next-day cron pattern.
    """
    dt = datetime.fromisoformat(week_start)
    # CBB is a daily-cadence sport (100+ D1 games/day mid-season). Narrow
    # window to 2 days so we don't grab entire week of picks at once.
    horizon = (dt + timedelta(days=2)).date().isoformat()

    # Try ncaab_game_context first (upcoming games with model context)
    r = requests.get(
        f'{SB}/rest/v1/ncaab_game_context'
    ...
    # Fallback: ncaab_game_results (may only exist post-schedule pull)
    r2 = requests.get(
...
def _team_matches(name: str, target: str) -> bool:
    ...
    Root cause of NCAAF Dimers scraper attaching "New Mexico State @
    Florida State" wp data to "Ball State @ Ohio State" (both away
    end in State, both home end in State). Same class fixed in
    pull_externals_ncaaf.py — mirroring here since NCAAB has even more
    generic-suffix overlap (Wildcats, Bulldogs, Tigers, State)."""
    ...
    GENERIC_SUFFIX = {'state','university','college','tech','a&m','a&t',
                      ...
                      'wildcats','bulldogs','tigers','eagles','panthers',
                      'lions','bears','cardinals','warriors','hawks'}
    n_last = name.split()[-1]
...
def find_game_id(slate: list, home_hint: str, away_hint: str) -> Optional[str]:
    """Zero-fail NCAAB game-id lookup.

    2026-09-03 REFACTOR: user directive after NCAAF Dimers mis-attribution
    (Ball State/New Mexico State bug) — no more loose fallback matching
    in college sports where hundreds of teams share suffixes. Same
    architecture as pull_externals_ncaaf.find_game_id.

    All-or-nothing:
      1. Resolve both hints via team_resolver.resolve_or_log_ncaab
      2. Both must resolve AND both must exact-match a slate game
      3. Miss → return None + gap is logged for triage
         (better zero external than wrong external — March Madness
         mis-attributions were the poster child for this class of bug)
    """
    try:
        from team_resolver import resolve_or_log_ncaab, resolve_ncaab_team
        canon_home = resolve_or_log_ncaab(home_hint, source='external_scrape')
        canon_away = resolve_or_log_ncaab(away_hint, source='external_scrape')
        if not (canon_home and canon_away):
            return None
        # 2026-09-03: canonicalize slate too — odds pull may store variant
        # names ('Duke Blue Devils') that don't literal-match resolver
        # canonical ('Duke'). See NCAAF find_game_id for full rationale.
        for g in slate:
            slate_home = resolve_ncaab_team(g.get('home_team') or '') or g.get('home_team')
            slate_away = resolve_ncaab_team(g.get('away_team') or '') or g.get('away_team')
...
def find_game_side(slate: list, team_hint: str) -> tuple[Optional[str], Optional[str]]:
    """Zero-fail single-team lookup for one-side-only NCAAB scrapers."""
    try:
        from team_resolver import resolve_or_log_ncaab, resolve_ncaab_team
        canon = resolve_or_log_ncaab(team_hint, source='external_scrape')
...
def load_ncaab_alias_map() -> dict:
    """Map full team name → canonical abbrev via ncaab_team_aliases."""
    r = requests.get(
        f'{SB}/rest/v1/ncaab_team_aliases?select=canonical_name,full_name,odds_api_name',
        headers=H_READ, timeout=15,
    )
...
def fetch_dimers(slate: list, game_date: str, aliases: dict) -> tuple:
    """Dimers CBB — same layout as MLB (Playwright helper)."""
    try:
        from _playwright_helper import render_page
    except ImportError:
        print('  ⚠ Dimers: _playwright_helper not importable')
        return [], 500
    text, err = render_page('https://www.dimers.com/bet-hub/cbb/schedule')
    if err == 'unavailable':
    ...
    for m in chunk_re.finditer(text):
        ...
        # 2026-09-03 ZERO-FAIL: route through find_game_id
        ...
        picks.append(ExternalPick(
            game_id=gid, sport='NCAAB', game_date=game_date, source='dimers',
...
def fetch_action(slate: list, game_date: str, aliases: dict) -> tuple:
    ...
    # Pre-emptive here: NCAAB is offseason, so this is untested against a live
    # slate and must be re-checked at the 11-03 launch.
    ...
    for m in chunk_re.finditer(text):
        ...
        picks.append(ExternalPick(
            game_id=gid, sport='NCAAB', game_date=game_date, source='action',
...
def fetch_pickswise(slate: list, game_date: str, aliases: dict) -> tuple:
    """Pickswise CBB — star ratings + moneyline/total/rl patterns.
    ...
    for m in ml_re.finditer(text):
        ...
        # 2026-09-03 ZERO-FAIL single-team lookup
        ...
        picks.append(ExternalPick(
            game_id=gid, sport='NCAAB', game_date=game_date, source='pickswise',
...
def fetch_oddscrowd(slate: list, game_date: str, aliases: dict) -> tuple:
    """OddsCrowd h2h/rl/total for NCAAB. Shared module handles the basketball
    page (mixed with NBA); slate + team-name matching filters cleanly."""
...
def run_pull(game_date: str, sources: list, triggered_by: str,
    ...
    print(f'\n=== NCAAB external pull · {game_date} · {triggered_by} ===')
    slate = load_slate(game_date)
    print(f'  slate: {len(slate)} games in ±2d window')
    if not slate:
        print('  ⚠ no NCAAB games in window — abort')
        return {'games': 0, 'sources_pulled': 0, 'picks_written': 0}

    aliases = load_ncaab_alias_map()
    ...
    # Compute alignment + oddscrowd snapshot for NCAAB app UX layer
    if not dry_run and summary['picks_written'] > 0:
        try:
            from compute_align_status_ncaab import run as compute_align_run
            print('\n=== Alignment + oddscrowd snapshot (NCAAB) ===')
            compute_align_run(game_date=game_date, dry_run=False)
        except Exception as e:
            print(f'  ⚠ compute_align_status_ncaab failed: {type(e).__name__}: {e}')
...
def main():
    ...
    if args.source:
        ...
        triggered_by = 'cron:evening_ncaab'
