-- team_stats_rolling: NBA block — kill hardcoded VALUES map, honest labels
-- (2026-09-01, part of the NBA root-cause square-away)
--
-- Coordinated with:
--   * 20260901i_nba_team_stats_team_name.sql (adds team_name column,
--     purges truncated abbrevs)
--   * nba_elo.py save_ratings rewrite (populates real ESPN abbrev + team_name)
--
-- Two changes to the NBA block:
--
--   1. HONEST LABELS. Prior version labeled the columns 'Off Rating' /
--      'Def Rating' / 'Net Rating' — but nba_elo.py writes PPG (points
--      per game), NOT per-100-possession ratings. Users seeing "Off
--      Rating 115" would think that's the real basketball-reference
--      metric; it's actually just PPG. Rename display_labels to
--      'Points/G', 'Points Allowed/G', 'Net Pts/G'. The internal
--      stat_keys stay stable (backward compat) — only the user-facing
--      display strings change.
--      When we later ship a four-factors + pace puller, we can either
--      overwrite the off_rating column with true per-100 values and
--      re-label back, or add new stat_keys for the pace-adjusted versions.
--
--   2. KILL HARDCODED VALUES MAP. Prior nba_abbrev_map CTE was a
--      hardcoded 30-team mapping from truncated abbrev ('Boston C')
--      to full name ('Boston Celtics'). Now that nba_team_stats has a
--      real team_name column populated by ESPN, we JOIN on team_name
--      directly. Cleaner + self-updating when NBA teams change (rare
--      but possible with expansion / relocation).
...
-- ═════════════════════════════════════════════════════════════════════
-- NHL per-game unpivot (unchanged from 20260901g)
-- ═════════════════════════════════════════════════════════════════════
nhl_home_rows AS (
  SELECT
    home_team AS team,
...
nhl_all AS (
  SELECT * FROM nhl_home_rows UNION ALL SELECT * FROM nhl_away_rows
),
nhl_season_avgs AS (
  SELECT
    team, season,
...
base_stats AS (
  ...
  -- ═════════════════════════════════════════════════════════════════
  -- NBA — REWRITTEN 2026-09-01 (root-cause square-away):
  --   * JOIN on team_name (populated by rewritten nba_elo.py from
  --     ESPN) instead of the hardcoded VALUES map on truncated abbrev
  --   * HONEST LABELS: off_rating column → 'Points/G' display, since
  --     nba_elo writes PPG (no pace adjustment yet). Same for def/net.
  --     When four-factors puller ships, columns get real ratings and
  --     labels flip back to 'Off Rating' or new stat_keys added.
  -- ═════════════════════════════════════════════════════════════════
  UNION ALL
  SELECT 'NBA', s.team_name,
    NULLIF(SPLIT_PART(s.season, '-', 1), '')::INT, 'points_pg',
    ROUND(s.off_rating::NUMERIC, 1), 'higher', 'Points/G', ''
  FROM public.nba_team_stats s
  WHERE s.off_rating IS NOT NULL AND s.team_name IS NOT NULL
  UNION ALL
  SELECT 'NBA', s.team_name,
    NULLIF(SPLIT_PART(s.season, '-', 1), '')::INT, 'points_allowed_pg',
    ROUND(s.def_rating::NUMERIC, 1), 'lower', 'Points Allowed/G', ''
  FROM public.nba_team_stats s
  WHERE s.def_rating IS NOT NULL AND s.team_name IS NOT NULL
  UNION ALL
  SELECT 'NBA', s.team_name,
    NULLIF(SPLIT_PART(s.season, '-', 1), '')::INT, 'net_pts_pg',
    ROUND(s.net_rating::NUMERIC, 1), 'higher', 'Net Pts/G', ''
  FROM public.nba_team_stats s
  WHERE s.net_rating IS NOT NULL AND s.team_name IS NOT NULL
  UNION ALL
  SELECT 'NBA', s.team_name,
    NULLIF(SPLIT_PART(s.season, '-', 1), '')::INT, 'wins',
    s.wins::NUMERIC, 'higher', 'Wins', ''
  FROM public.nba_team_stats s
  WHERE s.wins IS NOT NULL AND s.wins > 0 AND s.team_name IS NOT NULL
  UNION ALL
  -- Four-factors + pace (populated by future puller; stays NULL until then)
  SELECT 'NBA', s.team_name,
    NULLIF(SPLIT_PART(s.season, '-', 1), '')::INT, 'pace',
    ROUND(s.pace::NUMERIC, 1), 'higher', 'Pace', ''
  FROM public.nba_team_stats s
  WHERE s.pace IS NOT NULL AND s.team_name IS NOT NULL
  UNION ALL
  SELECT 'NBA', s.team_name,
    NULLIF(SPLIT_PART(s.season, '-', 1), '')::INT, 'efg_pct',
    ROUND((s.efg_pct * 100)::NUMERIC, 1), 'higher', 'eFG %', '%'
  FROM public.nba_team_stats s
  WHERE s.efg_pct IS NOT NULL AND s.team_name IS NOT NULL
  UNION ALL
  SELECT 'NBA', s.team_name,
    NULLIF(SPLIT_PART(s.season, '-', 1), '')::INT, 'tov_pct',
    ROUND((s.tov_pct * 100)::NUMERIC, 1), 'lower', 'TOV %', '%'
  FROM public.nba_team_stats s
  WHERE s.tov_pct IS NOT NULL AND s.team_name IS NOT NULL
  UNION ALL
  SELECT 'NBA', s.team_name,
    NULLIF(SPLIT_PART(s.season, '-', 1), '')::INT, 'opp_efg_pct',
    ROUND((s.opp_efg_pct * 100)::NUMERIC, 1), 'lower', 'Opp eFG %', '%'
  FROM public.nba_team_stats s
  WHERE s.opp_efg_pct IS NOT NULL AND s.team_name IS NOT NULL
